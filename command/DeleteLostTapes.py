...
# DeleteLostTapes.py
#   Description:
#       Sends the command to delete the specified tapes from the
#       BlackPearl database.
#====================================================================
...
def byBarcode(blackpearl, barcode, logbook):
    try:
        logbook.INFO("Deleting tape with barcode " + barcode)

        return blackpearl.deleteLostTape(barcode, logbook)
...
def fromFile(blackpearl, file_path, max_moves, logbook):
    try:
        logbook.INFO("Deleting tapes listed in " + file_path + " up to the maximum specified (" + str(max_moves) + ") tapes.")
    ...
    return "Delete lost tape command sent for (" + str(stats[1]) + ") tapes."

def fromFilteredList(blackpearl, filter_params, max_moves, logbook):
    try:
        logbook.INFO("Deleting tapes that match filter [" + filter_params + "] up to the maximum specified (" + str(max_moves) + ") tapes.")
    ...
    stats = sendDeletes(blackpearl, tape_list, max_moves, logbook)

    return "Delete lost tape command sent for (" + str(stats[1]) + ") tapes."

def sendDeletes(blackpearl, tape_list, max_moves, logbook):
    counter = 0
    success = 0
    failed = 0
    ...
    while(counter < len(tape_list) and success < int(max_moves)):
        if("barcode" not in tape_list[counter] and "bar_code" not in tape_list[counter]):
            ...
            if("Unable" in result or "Invalid State" in result):
                failed += 1
                print(result)
            else:
                success += 1
    ...
    logbook.INFO("Successfully deleted (" + str(success) + ") tapes.")
    logbook.INFO("Failed to delete (" + str(failed) + ") tapes.")
